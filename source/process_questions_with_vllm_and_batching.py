...
def generate_answers(input_prompts: List[str], large_language_model: LLM,
    ...
    for prompt_idx in range(len(input_prompts)):
        output_ids = outputs[prompt_idx].outputs[0].token_ids
        think_end_pos = 0
        if THINKING_END_TOKEN in llm_tokenizer.vocab:
            try:
                think_end_pos = len(output_ids) - output_ids[::-1].index(llm_tokenizer.vocab[THINKING_END_TOKEN])
...
def vectorize_search_queries(search_queries: List[str],
    ...
    tokenized_inputs = sent_emb_tokenizer(
        [('search_query: ' + cur_query) for cur_query in search_queries],
...
def find_relevant_chunks_with_vector_search(search_query_vector: np.ndarray,
                                            vector_db: AnnoyIndex,
                                            top_k: Optional[int]=VECTOR_TOP_K) -> List[int]:
    ndim = len(search_query_vector.shape)
    if ndim != 1:
        raise ValueError(f'The search query vector dimension in wrong! Expected 1-D, got {ndim}-D.')
    chunk_indices = vector_db.get_nns_by_vector(
        vector=search_query_vector,
        n=top_k, search_k=-1, include_distances=False
    )
...
def prepare_messages_for_search(user_question: str,
                                llm_tokenizer: Union[PreTrainedTokenizer, PreTrainedTokenizerFast],
                                add_json: Optional[bool] = False) -> Union[str, Tuple[str, Dict[str, Union[str, List[List[str]]]]]]:
    messages = [
        {
            'role': 'system',
    ...
    for cur_example in EXAMPLES_OF_SEARCH_QUERIES:
        user_query = SEARCH_USER_PROMPT.format(user_question=cur_example['question'])
        ...
        history.append([user_query, llm_response])
    messages.append({
        'role': 'user',
    ...
    if add_json:
        jsonified_sample = {
            ...
            'query': SEARCH_USER_PROMPT.format(user_question=user_question),
            'history': history
        }
        del history
        return text, jsonified_sample
    del history
    return text
...
def main():
    ...
    parser.add_argument('--gpu', dest='gpu_mem_part', type=float, required=False,
                        default=0.9, help='The GPU memory part for the vLLM-based inference of main LLM.')
    ...
    parser.add_argument('--debug', dest='debug_questions', type=int, required=False, default=None,
                        help='The questions number for debug.')
    ...
    if args.gpu_mem_part > MAX_GPU_MEM_PART_FOR_LLM:
        err_msg = (f'The GPU memory part for the vLLM-based inference of main LLM is too large '
                   f'(greater than {MAX_GPU_MEM_PART_FOR_LLM})!')
        qa_logger.error(err_msg)
        raise IOError(err_msg)
    if args.gpu_mem_part < MIN_GPU_MEM_PART_FOR_LLM:
        err_msg = (f'The GPU memory part for the vLLM-based inference of main LLM is too small '
                   f'(less than {MIN_GPU_MEM_PART_FOR_LLM})!')
        qa_logger.error(err_msg)
        raise IOError(err_msg)
    ...
    try:
        if args.debug_questions is None:
            benchmark = load_dataset(args.dataset_name, split=args.split_name)
        else:
            benchmark = load_dataset(args.dataset_name, split=args.split_name + f'[:{args.debug_questions}]')
    ...
    samples_for_rlhf: List[Dict[str, Union[str, List[List[str]]]]] = []
    saved_indices_of_samples_for_rlhf = []
    search_queries_for_all_questions = []
    list_of_input_prompts = []
    nonempty_question_indices = []
    list_of_relevant_documents = []
    ...
    for question_count, cur_question in enumerate(test_questions):
        ...
        prompt_for_search, jsonified_promt = prepare_messages_for_search(
        ...
        if (len(output_dataset_fname) > 0) and args.evaluate_quality:
            ...
            new_sample_for_rlhf['rejected_response'] = search_queries
        ...
        search_queries_for_all_questions.append(search_queries)
        if len(list_of_search_queries) == 0:
            ...
            if not is_question_meaningful:
                ...
                list_of_search_queries.append(cur_question)
                try:
                    vectorized_search_queries = vectorize_search_queries(
                        search_queries=list_of_search_queries,
                    ...
                    finalize_vllm()
                    raise
                for query_count in range(vectorized_search_queries.shape[0]):
                    try:
                        indices_from_vector_search = find_relevant_chunks_with_vector_search(
                            search_query_vector=vectorized_search_queries[query_count],
                    ...
                    try:
                        indices_from_bm25 = find_relevant_chunks_with_bm25_search(
                            user_question=list_of_search_queries[query_count],
                ...
                if number_of_chunks_for_llm > 0:
                    ...
                    nonempty_question_indices.append(question_count)
                    list_of_input_prompts.append(prompt_for_qa)
                    list_of_relevant_documents.append(relevant_documents)
                    if (len(output_dataset_fname) > 0) and args.evaluate_quality:
                        new_sample_for_rlhf = copy.copy(jsonified_promt)
                        new_sample_for_rlhf['history'] = []
                        saved_indices_of_samples_for_rlhf.append(len(samples_for_rlhf))
                        samples_for_rlhf.append(new_sample_for_rlhf)
                        del new_sample_for_rlhf
                    del jsonified_promt
                del union_of_relevant_indices
        del list_of_search_queries
    ...
    torch.cuda.empty_cache()
    gc.collect()
    qa_logger.info('The question processing is successfully finished.')
    if len(list_of_input_prompts) != len(list_of_relevant_documents):
        err_msg = (f'The input prompts list does not correspond to the relevant documents list. '
                   f'{len(list_of_input_prompts)} != {len(list_of_relevant_documents)}')
        finalize_vllm()
        qa_logger.info(err_msg)
        raise RuntimeError(err_msg)
    if len(list_of_input_prompts) != len(nonempty_question_indices):
        err_msg = (f'The input prompts list does not correspond to the non-empty questions list. '
                   f'{len(list_of_input_prompts)} != {len(nonempty_question_indices)}')
        finalize_vllm()
        qa_logger.info(err_msg)
        raise RuntimeError(err_msg)
    if not (set(nonempty_question_indices) <= set(range(num_questions))):
        err_msg = (f'The non-empty questions list do not correspond to the source questions!\n'
                   f'{sorted(list(set(nonempty_question_indices) - set(range(num_questions))))}')
        finalize_vllm()
        qa_logger.info(err_msg)
        raise RuntimeError(err_msg)
    if len(saved_indices_of_samples_for_rlhf) > 0:
        if len(list_of_input_prompts) != len(saved_indices_of_samples_for_rlhf):
            err_msg = (f'The input prompts list does not correspond to the RLHF samples. '
                       f'{len(list_of_input_prompts)} != {len(saved_indices_of_samples_for_rlhf)}')
            finalize_vllm()
            qa_logger.info(err_msg)
            raise RuntimeError(err_msg)
    info_msg = f'The answer generation for {len(list_of_input_prompts)} questions with LLM is started.'
    qa_logger.info(info_msg)
    nonempty_answers = generate_answers(
        input_prompts=list_of_input_prompts,
        large_language_model=main_llm,
        llm_tokenizer=llm_tokenizer,
        sampling_params=llm_sampling_params
    )
    info_msg = f'The answer generation for {len(list_of_input_prompts)} questions with LLM is successfully finished.'
    qa_logger.info(info_msg)
    results = dict()
    for test_idx, answer, relevant_docs in zip(nonempty_question_indices, nonempty_answers, list_of_relevant_documents):
        results[test_idx] = {
            'answer': answer,
            'document': relevant_docs
        }
        if args.evaluate_quality:
            results[test_idx]['reference_answer'] = reference_answers[test_idx]
    if len(saved_indices_of_samples_for_rlhf) > 0:
        for rlhf_index, rejected_answer in zip(saved_indices_of_samples_for_rlhf, nonempty_answers):
            samples_for_rlhf[rlhf_index]['rejected_response'] = rejected_answer
    if set(nonempty_question_indices) != set(results.keys()):
        err_msg = (f'Non-empty question indices do not correspond to results!\n'
                   f'{sorted(list(set(results.keys()) - set(nonempty_question_indices)))}')
        finalize_vllm()
        qa_logger.error(err_msg)
        raise RuntimeError(err_msg)
    for test_idx in sorted(list(set(range(num_questions)) - set(nonempty_question_indices))):
        results[test_idx] = {
    ...
    with codecs.open(submit_fname, mode='w', encoding='utf-8', buffering=0) as dst_fp:
        ...
        if args.evaluate_quality:
            data_writer.writerow(['question', 'search queries', 'answer', 'document',
                                  'reference_answer', 'chrF++'])
            for test_idx in range(num_questions):
                instant_quality_score = chrf.compute(
                    predictions=[results[test_idx]['answer']],
                    references=[[results[test_idx]['reference_answer']]]
                )['score']
                data_writer.writerow([
                    test_questions[test_idx],
                    search_queries_for_all_questions[test_idx],
                    results[test_idx]['answer'],
                    results[test_idx]['document'],
                    results[test_idx]['reference_answer'],
                    round(instant_quality_score, 4)
                ])
                list_of_generated_answers.append(results[test_idx]['answer'])
        else:
            data_writer.writerow(['question', 'search queries', 'answer', 'document'])
            for test_idx in range(num_questions):
                data_writer.writerow([
    ...
    if (len(output_dataset_fname) > 0) and (len(samples_for_rlhf) > 0):
        filtered_samples_for_rlhf = list(filter(
            lambda it1: ('rejected_response' in it1) and (len(it1['rejected_response']) > 0),
            samples_for_rlhf
        ))
        with codecs.open(output_dataset_fname, mode='w', encoding='utf-8') as jsonl_fp:
            for it2 in filtered_samples_for_rlhf:
                jsonl_fp.write(json.dumps(it2, ensure_ascii=False) + '\n')
