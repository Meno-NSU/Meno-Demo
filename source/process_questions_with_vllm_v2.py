...
RERANKING_THRESHOLD = -0.07

QA_SYSTEM_PROMPT = ('Вы - искусственный интеллект по имени Менон, разработанный Новосибирским государственным '
                    'университетом. Вы - эксперт в области науки, техники и юриспруденции. Вы безукоризненно вежливы, '
                    'а ваши ответы на сложные вопросы человека всегда логичны и точны. Вы способны разобраться даже в '
                    'самой сложной документации, чтобы помочь человеку извлечь из этой документации полезные сведения '
                    'для ответа на вопрос. Вы точно, понятно и детально отвечаете пользователю на русском языке.')
QA_USER_PROMPT = ('Внимательно проанализируйте найденные фрагменты следующих документов, представленные в формате JSON:'
                  ...
                  'заданному вопросу информацию. Если же вопрос человека вoобще не связан ни с одним из найденных '
...
EXAMPLES_OF_SEARCH_QUERIES = (
    ...
    },
   {
        ...
        'search_queries': (
            'капитальный ремонт проведение', # 0
            'капитальный ремонт очередность', # 1
            'капитальный ремонт критерии очередности' # 2
        )
    },
    {
    ...
    },
    {
        ...
        'search_queries': (
            'когда проводится капитальный ремонт основания критерии периодичность нормативные документы', # 0
            'капитальный ремонт многоквартирного дома когда должен проводиться сроки по региональной программе', # 1
            'признаки необходимости капитального ремонта перечень работ отличие от текущего ремонта когда требуется' # 2
        )
    },
    {
...
def rerank_chunks(user_question: str, selected_chunks: List[int],
    ...
    scored_indices = sorted(
        list(filter(lambda x: x[1] > RERANKING_THRESHOLD, zip(selected_chunks_, scores))),
