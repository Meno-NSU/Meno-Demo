...
def prepare_messages_for_answering(user_question: str,
                                   context: Dict[str, Dict[str, Union[List[str], List[int]]]]) -> Dict[str, str]:
    structured_context = []
    ...
    del structured_context, documents
...
def prepare_bm25(chunks: List[str], stemmer: SnowballStemmer) -> BM25Okapi:
    stemmed_texts = [prepare_text_for_bm25(it, stemmer) for it in tqdm(chunks, desc='BM25 index building')]
...
def main():
    ...
    parser.add_argument('-o', '--output', dest='output_jsonl', type=str, required=True,
                        help='The output JSON-line file with prepared dataset.')
    args = parser.parse_args()

    output_jsonl_fname = os.path.normpath(args.output_jsonl)
    if not os.path.isfile(output_jsonl_fname):
        base_dir = os.path.dirname(output_jsonl_fname)
        if len(base_dir) > 0:
    ...
    try:
        ...
        raise
    num_chunks = len(chunk_list)
    chunk_lengths = [len(it['chunk_text']) for it in chunk_list]
    mean_chunk_length = round(np.mean(chunk_lengths))
    std_chunk_length = round(np.std(chunk_lengths))
    info_msg = (f'The text corpus is loaded from {text_corpus_fname}. This corpus consists of {num_chunks} chunks. '
                f'The mean chunk length (in characters) is {mean_chunk_length} ± {std_chunk_length}.')
    qa_logger.info(info_msg)
    ...
    with codecs.open(output_jsonl_fname, mode='w', encoding='utf-8', buffering=0) as dst_fp:
            ...
            if len(list_of_search_queries) == 0:
                ...
                if not is_question_meaningful:
                    ...
                    if number_of_chunks_for_llm > 0:
                        ...
                        qa_logger.info(info_msg)
                        new_sample_for_dataset = prepare_messages_for_answering(
                            user_question=cur_question,
                            context=prepared_context
                        )
                        new_sample_for_dataset['response'] = reference_answers[question_count]
                        new_sample_for_dataset['history'] = []
                        dst_fp.write(json.dumps(new_sample_for_dataset, ensure_ascii=False) + '\n')
                        del new_sample_for_dataset
...
if __name__ == '__main__':
    ...
    file_handler = logging.FileHandler('dataset_preparing_with_vllm.log')
